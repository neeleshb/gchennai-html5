...
function setColorToMainBox(color) {
	$('historyMainBoxExampleId').style.backgroundColor = color;
...
function generateMatrixCharacters() {
	var numC = 25; //Number of characters to generate.
	var topStart = 450; // The 'top' coordinate of the pixel from which character starts falling
	var topEnd = 650; // The 'top' coordinate of the pixel till which the character falls.
	var leftMin = 100; // The minimum value for the 'left' coordinate.
	var leftMax = 900; // The maximum value for the 'left' coordinate.
	
	for (var i = 0; i < numC; ++i) {
		var lft = Math.round(Math.random()*1000);
		if (lft < 100) lft = 100;
		else if (lft > 900) lft = 900;
		var node = document.createElement('span');
		var ch = Math.round(Math.random()*10);
		node.innerHTML = ch;
		node.classList.add('matrixAnimate');
		node.style['left'] = lft;
		$('matrixQuote').appendChild(node);
